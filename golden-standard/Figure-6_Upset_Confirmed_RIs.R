...
##DESCRIPTION: This script allow the analysis of evidence types combinations from RIs subsets
...
##OUTPUT: A upset plot of evidence types combinations found in RIs with Confirmed confidence level
...
# Built an Upset of binding evidence combinations found in Confirmed RIs Figure 6

combinationsBindingEvConfirmedRisAbundances<- df_RIs_set %>%
  filter(X19.confidenceLevel=="Confirmed") %>%
  select(X27.tfrsEv) %>%
  filter(X27.tfrsEv!="-") %>%
  count(X27.tfrsEv)
...
combinationsBindingEvConfirmedRisAbundances$X27.tfrsEv <- str_replace_all (combinationsBindingEvConfirmedRisAbundances$X27.tfrsEv, ",", "&")
combinationsBindingEvConfirmedRisAbundancesWithCutOff <- combinationsBindingEvConfirmedRisAbundances %>%
  filter(n>20)
evCombinationConfirmedNames <- combinationsBindingEvConfirmedRisAbundances$X27.tfrsEv 
...
upset(fromExpression(evCombinationConfirmedCounts), 
      ...
      point.size = .6, 
      line.size = .3)
grid.text("Binding evidence supporting confirmed RIs",x = 0.65, y=0.95, gp=gpar(fontsize=10))