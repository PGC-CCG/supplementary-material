...
##OUTPUT: A upset plot of evidence types combinations found in RIs with weak confidence level
...
# Built an Upset of binding evidence combinations found in weak RIs Figure 8

combinationsBindingEvWeakRisAbundances<- df_RIs_set %>%
  filter(X19.confidenceLevel=="Weak") %>%
  select(X27.tfrsEv) %>%
  filter(X27.tfrsEv!="-") %>%
  count(X27.tfrsEv)
...
combinationsBindingEvWeakRisAbundances$X27.tfrsEv <- str_replace_all (combinationsBindingEvWeakRisAbundances$X27.tfrsEv, ",", "&")
combinationsBindingEvWeakRisAbundancesWithCutOff <- combinationsBindingEvWeakRisAbundances %>%
  filter(n>20)
evCombinationWeakNames <- combinationsBindingEvWeakRisAbundances$X27.tfrsEv 
...
upset(fromExpression(evCombinationWeakCounts), 
      ...
      point.size = .6, 
      line.size = .3)
grid.text("Binding evidence supporting weak RIs",x = 0.65, y=0.95, gp=gpar(fontsize=10))
