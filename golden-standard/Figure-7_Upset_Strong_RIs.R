...
##OUTPUT: A upset plot of evidence types combinations found in RIs with Strong confidence level
...
# Built an Upset of binding evidence combinations found in strong RIs Figure 7

combinationsBindingEvStrongRisAbundances<- df_RIs_set %>%
  filter(X19.confidenceLevel=="Strong") %>%
  select(X27.tfrsEv) %>%
  filter(X27.tfrsEv!="-") %>%
  count(X27.tfrsEv)
...
combinationsBindingEvStrongRisAbundances$X27.tfrsEv <- str_replace_all (combinationsBindingEvStrongRisAbundances$X27.tfrsEv, ",", "&")
combinationsBindingEvStrongRisAbundancesWithCutOff <- combinationsBindingEvStrongRisAbundances %>%
  filter(n>20)
evCombinationStrongNames <- combinationsBindingEvStrongRisAbundances$X27.tfrsEv 
...
upset(fromExpression(evCombinationStrongCounts), 
      ...
      point.size = .6, 
      line.size = .3)
grid.text("Binding evidence supporting strong RIs",x = 0.65, y=0.95, gp=gpar(fontsize=10))